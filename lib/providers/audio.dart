...
Future<void> play(int chapter,int verse,GlobalKey<ScaffoldState> scaffoldKey)async{
  String dir = await getPath();
  File file = File('$dir/audio/$chapter/${chapter}_$verse.mp3');
  if(file.existsSync()){
  var _duration = await audioPlayer.setFilePath('$dir/audio/$chapter/${chapter}_$verse.mp3').catchError((onError)async{
  ...
  await audioPlayer.play().then((_)async{
   if(audioPlayer.playbackState==AudioPlaybackState.completed){
    ...
    scaffoldKey.currentState.showSnackBar(SnackBar(content: Text('Audio doesn\'t exist'),duration: Duration(seconds: 1),));
  }
}
